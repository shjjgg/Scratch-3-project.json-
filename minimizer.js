...
class Minimizer {
	...
	main(json, options={}) {
		if(typeof json !== "object") {
			throw new Error("第一个参数必须是对象（已解析的 json）");
		}
		//console.log(json);
	...
	findIdsMutations(json, ids, mutations) {
		...
		for(let sprite of sprites) {
			//console.log(sprite);
			if(Array.isArray(sprite)) throw new Error("尚不支持由数组表示的顶层积木");
					...
					for(let i=1; i<input.length; i++) {
						...
						if(Array.isArray(inputBlock)) {
							...
							if(inputBlock[0] < 4 || inputBlock[0] > 13) {
								throw new Error("遇到未实现的输入类型 " + inputBlock[0]);
							}
						} else {
	...
	applyIdsMutations(json, ids, mutations) {
		for(let id of ids) {
			if(id.newId) {
				if(id.location == "item") {
					if(id.object[id.item] !== id.id) throw new Error("需要替换的 ID 不存在（mode=item）");
					id.object[id.item] = id.newId;
				}
				if(id.location == "key") {
					if(!id.object[id.id]) throw new Error("需要替换的 ID 不存在（mode=key）");
		...
		for(let mutation of mutations) {
			...
			if(mutation.location == "item") {
				if(!mutation.object[mutation.item]) throw new Error("需要替换的 Mutation 不存在（mode=item）");
